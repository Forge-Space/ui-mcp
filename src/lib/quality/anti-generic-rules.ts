...
function validateSemanticColors(
  ...
  if (match) {
    errors.push(
      `Raw color literal "${match[0]}" found. ` +
        'Use semantic tokens (bg-primary, text-foreground, etc.)'
    );
  }
}

function validateAntiGenericMarkers(
  snippet: IComponentSnippet,
  errors: string[]
): void {
  ...
  if (markers.length < 2) {
    errors.push(
      `Requires 2+ antiGeneric markers, found ${markers.length}`
    );
  }
}

function validateCraftDetails(
  snippet: IComponentSnippet,
  errors: string[]
): void {
  ...
  if (details.length < 2) {
    errors.push(
      `Requires 2+ craftDetails, found ${details.length}`
    );
  }
}

function validateInspirationSource(
  snippet: IComponentSnippet,
  errors: string[]
): void {
...
function validateTypographyIntentionality(
  snippet: IComponentSnippet,
  warnings: string[]
): void {
  const allClasses = Object.values(snippet.tailwindClasses).join(' ');
  const hasTypo =
    /(?:font-|text-(?:xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl)|tracking-|leading-)/.test(
      allClasses
    );
  if (!hasTypo) {
    warnings.push(
      'No typography classes found — consider font/tracking/leading'
    );
  }
}
...
function validateNoPlaceholderContent(
  ...
  for (const pattern of PLACEHOLDER_PATTERNS) {
    if (pattern.test(snippet.jsx)) {
      errors.push(
        `Placeholder content detected matching ${pattern.source}`
      );
      return;
    }
  }
...
export function validateSnippetStrict(
  snippet: IComponentSnippet
): IValidationResult {
